...
async def line_callback(code: str = None, state: str = None, error: str = None, db: Session = Depends(get_db)):
    ...
    try:
        ...
        if user:
            ...
            if user.status != UserStatus.active:
                return RedirectResponse(url=f"{settings.FRONTEND_URL}/pending-approval")
            ...
            return RedirectResponse(url=f"{settings.FRONTEND_URL}/login?line_token={token}&role={user.sys_role.value}", status_code=302)
            ...
            return RedirectResponse(
                url=f"{settings.FRONTEND_URL}/select-role?token={reg_token}&name={display_name}"
            )

    except Exception as e:
        print(f"LINE Login Error: {e}")
        return RedirectResponse(url=f"{settings.FRONTEND_URL}/login?error=line_error")
